...
void diffCentreVoidFraction::setvoidFraction(double** const& mask,double**& voidfractions,double**& particleWeights,double**& particleVolumes,double**& particleV) const
{
    ...
    for(int index=0; index< particleCloud_.numberOfParticles(); index++)
    {
            ...
            {
                cellVol = voidfractionNext_.mesh().V()[cellI];    // voidfractionNext_ is a volScalarField, the member function mesh is??
                ...
                voidfractions[index][0] = 1 - particlefractionNext_[cellI];
